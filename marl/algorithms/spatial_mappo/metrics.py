"""SpatialMAPPOMetricsTracker calculating multi-agent rewards, losses, throughput, and Jain's fairness index for S-MAPPO."""
...
class SpatialMAPPOMetricsTracker:
    """Tracks S-MAPPO training telemetry, per-agent rewards, CNN critic loss, throughput, and fairness index."""
    ...
    def aggregate_multi_agent_metrics(
        ...
        """Aggregates per-agent and CNN critic metrics into summary metrics."""
        rewards_list = list(agent_rewards.values())
        mean_reward = float(np.mean(rewards_list)) if rewards_list else 0.0
        fairness = SpatialMAPPOMetricsTracker.compute_jains_fairness(rewards_list)

        metrics = {
            "mean_reward": mean_reward,
            "cnn_critic_loss": float(critic_loss),
